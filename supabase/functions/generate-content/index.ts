...
async function generateChapterContentWithHAL9(
  ...
  // Create a comprehensive prompt for HAL9
  const prompt = createHAL9Prompt(title, author, book_idea, chapter, content_depth, chapterNumber, toc.length);
  
  try {
    // Call HAL9 API
    const hal9Response = await fetch('https://api.hal9.com/v1/chat/completions', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a professional book writer and editor. Generate high-quality, engaging book content that is well-structured, informative, and appropriate for the target audience.'
          },
          {
            role: 'user',
            content: prompt
    ...
    const generatedContent = hal9Data.choices?.[0]?.message?.content;

    if (!generatedContent) {
      throw new Error('No content generated by HAL9 API');
    }
